 ...
 */
export const hello_pingkeng = async (request: Request, response: Response): Promise<void> => {
    /**
 ...
 */
export const pingkeng_hsieh = async (request: Request, response: Response): Promise<void> => {
    const baseurl = functions.config().baseurl.firebase; // resolves to https://cloudfunctions.net
    const data = {
        email: "pxh190002@utdallas.edu"
    };
