 ...
 */
export const hello_hammad = async (request: Request, response: Response): Promise<void> => {
    /**
 ...
 */
export const hammad_khan = async (request: Request, response: Response): Promise<void> => {
    const baseurl = functions.config().baseurl.firebase; // resolves to https://cloudfunctions.net
    const data = {
        email: "hxk180036@utdallas.edu"
    }
